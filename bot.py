...
if not TOKEN:
    raise ValueError("❌ Không tìm thấy TOKEN! Hãy thêm biến TOKEN trong Railway.")          # ← BƯỚC 5 sẽ thay
OWNER_ID = 8765391878                                # ← BƯỚC 7 sẽ thay
...
BOLD_ITALIC = {'A':'𝑨','B':'𝑩','C':'𝑪','D':'𝑫','E':'𝑬','F':'𝑭','G':'𝑮','H':'𝑯','I':'𝑰','J':'𝑱','K':'𝑲','L':'𝑳','M':'𝑴','N':'𝑵','O':'𝑶','P':'𝑷','Q':'𝑸','R':'𝑹','S':'𝑺','T':'𝑻','U':'𝑼','V':'𝑽','W':'𝑾','X':'𝑿','Y':'𝒀','Z':'𝒁','a':'𝒂','b':'𝒃','c':'𝒄','d':'𝒅','e':'𝒆','f':'𝒇','g':'𝒈','h':'𝒉','i':'𝒊','j':'𝒋','k':'𝒌','l':'𝒍','m':'𝒎','n':'𝒏','o':'𝒐','p':'𝒑','q':'𝒒','r':'𝒓','s':'𝒔','t':'𝒕','u':'𝒖','v':'𝒗','w':'𝒘','x':'𝒙','y':'𝒚','z':'𝒛'}
...
def convert_phrase(phrase, font, mode):
    """Xử lý cụm từ có nhiều từ (có dấu cách)"""
    if not phrase:
        return phrase
    words = phrase.split()                    # tách thành từng từ
...
def process_text(text, user_data, mode="full", global_contact="LH: 076.6482.506"):
    # Sắp xếp cụm dài trước để tránh lỗi thay thế
    ...
    for phrase in phrases:
        if phrase:
            ...
            result = result.replace(phrase, converted)   # thay thế chính xác cụm từ
    
    # Luôn thêm dòng LH ở cuối và in đậm theo font
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"🔑 USER ID của bạn là: {user_id}")
...
async def font_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("Bold Serif", callback_data="font_serif")],
                [InlineKeyboardButton("Bold Sans Serif", callback_data="font_sans")],
                [InlineKeyboardButton("Bold Italic", callback_data="font_italic")]]
    await update.message.reply_text("Chọn font bạn muốn sử dụng:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def m1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = process_text(text, users[user_id], "full", global_contact)
...
async def m2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = process_text(text, users[user_id], "first", global_contact)
    await update.message.reply_text(result)
...
async def m3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = process_text(text, users[user_id], "first_last", global_contact)
    await update.message.reply_text(result)

async def setlh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != str(OWNER_ID):
        await update.message.reply_text("🚫 Chỉ chủ bot mới thay được dòng LH!")
        return
    ...
    save_data(data)
    await update.message.reply_text(f"✅ Đã đổi dòng LH thành:\n{new_contact}")
...
async def handle_normal_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = str(update.effective_user.id)
    if user_id not in users: users[user_id] = {"words": [], "font": "sans"}
    result = process_text(update.message.text, users[user_id], "full", global_contact)
    await update.message.reply_text(result)
...
def main():
    global OWNER_ID
    if OWNER_ID is None:
        print("⚠️  Chưa có OWNER_ID. Chạy /start trong Telegram để lấy ID, sau đó thay vào code và chạy lại.")
