...
# Các chữ luôn dùng Mathematical Bold (kể cả hoa)
...
def apply_math_font(char: str) -> str:
    """Áp dụng font cho ký tự đầu/cuối theo quy tắc mới"""
    if char in ALWAYS_BOLD_CHARS:
        return MATH_BOLD.get(char, char)
    if char.isupper():
        return MATH_MONOSPACE.get(char, char)
    else:
        # Ngẫu nhiên Monospace hoặc Bold cho chữ thường
...
async def m5(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = apply_special_map(text) + "\n" + apply_special_map(global_contact)   # tạm dùng chung logic m4
    await update.message.reply_text(result)
...
/m1 <text> - In đậm toàn bộ theo font
/m2 <text> - In đậm chữ cái đầu
/m3 <text> - In đậm chữ cái đầu + cuối
/m4 <text> - Đổi ký tự đặc biệt sang kiểu Cyrillic/Greek
/m5 <text> - Chỉ đổi từ đã add sang kiểu đặc biệt
/m6 <text> - Từ đã add dùng special map, chữ h,g,n,t,v luôn Bold, chữ đầu/cuối khác dùng Mathematical font ngẫu nhiên
...
def main():
    ...
    print("🚀 Bot BoldGen đang chạy... (/m6 đã cập nhật)")
    app.run_polling()
