...
def process_text(text: str, user_data: dict, mode: str = "full", global_contact: str = "LH: 076.6482.506") -> str:
...
async def m1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("⚠️ Nhập nội dung sau /m1")
        return
...
async def m2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = " ".join(context.args) or (update.message.text.split(maxsplit=1)[1] if len(update.message.text.split()) > 1 else "")
    if not text:
        await update.message.reply_text("⚠️ Nhập nội dung sau /m2")
        return
...
async def m3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = " ".join(context.args) or (update.message.text.split(maxsplit=1)[1] if len(update.message.text.split()) > 1 else "")
    if not text:
        await update.message.reply_text("⚠️ Nhập nội dung sau /m3")
        return
...
def main():
    ...
    app.add_handler(CommandHandler("font", lambda u, c: u.message.reply_text(
        "Chọn font bạn muốn sử dụng:", 
    ...
    print("🚀 Bot BoldGen đang chạy... (Phiên bản tương thích v22)")
    app.run_polling()
