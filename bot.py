...
MATH_MONOSPACE = {'A':'𝙰','B':'𝙱','C':'𝙲','D':'𝙳','E':'𝙴','F':'𝙵','G':'𝙶','H':'𝙷','I':'𝙸','J':'𝙹','K':'𝙺','L':'𝙻','M':'𝙼','N':'𝙽','O':'𝙾','P':'𝙿','Q':'𝚀','R':'𝚁','S':'𝚂','T':'𝚃','U':'𝚄','V':'𝚅','W':'𝚆','X':'𝚇','Y':'𝚈','Z':'𝚉',
                  'a':'𝚊','b':'𝚋','c':'𝚌','d':'𝚍','e':'𝚎','f':'𝚏','g':'𝚐','h':'𝚑','i':'𝚒','j':'𝚓','k':'𝚔','l':'𝚕','m':'𝚖','n':'𝚗','o':'𝚘','p':'𝚙','q':'𝚚','r':'𝚛','s':'𝚜','t':'𝚝','u':'𝚞','v':'𝚟','w':'𝚠','x':'𝚡','y':'𝚢','z':'𝚣'}

MATH_BOLD = {'A':'𝐀','B':'𝐁','C':'𝐂','D':'𝐃','E':'𝐄','F':'𝐅','G':'𝐆','H':'𝐇','I':'𝐈','J':'𝐉','K':'𝐊','L':'𝐋','M':'𝐌','N':'𝐍','O':'𝐎','P':'𝐏','Q':'𝐐','R':'𝐑','S':'𝐒','T':'𝐓','U':'𝐔','V':'𝐕','W':'𝐖','X':'𝐗','Y':'𝐘','Z':'𝐙',
             'a':'𝐚','b':'𝐛','c':'𝐜','d':'𝐝','e':'𝐞','f':'𝐟','g':'𝐠','h':'𝐡','i':'𝐢','j':'𝐣','k':'𝐤','l':'𝐥','m':'𝐦','n':'𝐧','o':'𝐨','p':'𝐩','q':'𝐪','r':'𝐫','s':'𝐬','t':'𝐭','u':'𝐮','v':'𝐯','w':'𝐰','x':'𝐱','y':'𝐲','z':'𝐳'}

ALWAYS_BOLD_CHARS = {'h', 'g', 'n', 't', 'v', 'H', 'G', 'N', 'T', 'V'}
...
def remove_viet_accent(text: str) -> str:
    return ''.join(VIET_TO_NOACCENT.get(c, c) for c in text)
...
def apply_math_font(char: str) -> str:
    if char in ALWAYS_BOLD_CHARS:
        return MATH_BOLD.get(char, char)
    if char.isupper():
...
def process_text_m6(text: str, user_data: dict, global_contact: str) -> str:
    ...
    for word in words:
        ...
        for i, char in enumerate(word):
            ...
            lower = char.lower()
            if lower in SPECIAL_MAP_LOWER or char in ALWAYS_BOLD_CHARS:
                if is_first or is_last:
                    new_word.append(apply_math_font(char))
                else:
...
def process_text_m7(text: str, user_data: dict, global_contact: str) -> str:
    text_no_accent = remove_viet_accent(text)
    contact_no_accent = remove_viet_accent(global_contact)
    return process_text_m6(text_no_accent, user_data, contact_no_accent)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "start_button":
        keyboard = [
            [InlineKeyboardButton("🔤 Chọn Font", callback_data="choose_font")],
            [InlineKeyboardButton("📖 Hướng dẫn sử dụng", callback_data="show_help")]
        ]
