...
def process_text_m4(text: str, global_contact: str) -> str:
    """ /m4 : Toàn bộ văn bản dùng special map """
    result = apply_special_map(text)
    bold_contact = apply_special_map(global_contact)
    return result + "\n" + bold_contact

def process_text_m5(text: str, user_data: dict, global_contact: str) -> str:
    """ /m5 : Chỉ các từ đã thêm vào danh sách dùng special map """
    phrases = sorted(user_data.get("words", []), key=len, reverse=True)
    result = text
    for phrase in phrases:
        if phrase:
            converted = apply_special_map(phrase)
...
async def m4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = " ".join(context.args) or (update.message.text.split(maxsplit=1)[1] if len(update.message.text.split()) > 1 else "")
    if not text:
        await update.message.reply_text("⚠️ Nhập nội dung sau /m4")
        return
    result = process_text_m4(text, global_contact)
    await update.message.reply_text(result)
...
async def m5(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = " ".join(context.args) or (update.message.text.split(maxsplit=1)[1] if len(update.message.text.split()) > 1 else "")
    if not text:
        await update.message.reply_text("⚠️ Nhập nội dung sau /m5")
        return
    result = process_text_m5(text, users[user_id], global_contact)
    await update.message.reply_text(result)
